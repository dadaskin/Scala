object E_ResistorTolerance {


  private sealed trait Resistor {
    def resistance: Double
    def tolerance: Double
  }  

  private enum E6(val resistance: Double) extends Resistor {
    val tolerance = 0.2
  ...
  private enum E12(val resistance: Double) extends Resistor {
    val tolerance = 0.1
  ...
  private def range(resistors: List[Resistor]): (Double, Double) =
  ...
  def main(args: Array[String]): Unit = {
    println("Range is: " + range(List(E12.TenOhm, E6.TenOhm)))
  }
}